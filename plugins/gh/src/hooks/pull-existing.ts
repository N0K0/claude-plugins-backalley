...
import { serializeIssue, ensureLocation } from '../tools/issue-files.js';
...
async function main() {
  ...
  for (const file of numbered) {
    try {
      ...
      const filePath = await ensureLocation(issuesDir, file.number, issue.title, issue.state, file.path);
